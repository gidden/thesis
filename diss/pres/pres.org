...
*** Motivation
#+begin_quote
A tool is needed that can determine isotope-specific resource flows for
arbitrary numbers and types of facilities where demand can be met by fungible
resources and supply is constrained both by resource quantity and quality.
#+end_quote
...
:BEAMER_col: 0.4
:END:
- ask for (complex) resource
- collection of bids in a portfolio
- mutual bids
- exclusive bids
- preferences
...
- collection of offers in a portfolio
...
- Possible trades known
- Preferences for trades known
- Constrained-graph representation of exchange

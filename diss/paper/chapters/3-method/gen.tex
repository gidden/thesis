...
\cite{bairiot2003status}. Note that no large-scale fast reactors have been
fueled by Thorium-based fuels. Accordingly, Thorium-related values are broad
generalizations. The purpose of including another fuel type is to expand on the
complexity of possible connections between facilities in a given fuel cycle,
while including somewhat realistic constraining values. The constraining values
used by any individual analyst will vary, perhaps greatly, and thus only

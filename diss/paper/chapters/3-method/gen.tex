...
is modeled wihtin the formulation by choosing preferences such that there is a
...
by a parameter defined as the ratio of TMOX to UOX supporters, $r_{s,
  tmox, uox}$, such that the number of UOX and TMOX supporters is

\begin{equation}
n_{s, uox} = \frac{n_{s, th}}{1 + r_{s, tmox, uox}}
\end{equation}
...
for generating an inventory constrainting value by deriving it from the process
...
to the Rokkassho plant in Japan \cite{heinonen2010}. Given the request
...
can enter the facility. Thus, a repostiory's monthly request quantity is
...
assembly-commodity breakdown would be calulated as
...
assemblies, the remaining supply generation methodology is identical. If \ffx is
...
to a single assembly that should not be split between supporting facilites.

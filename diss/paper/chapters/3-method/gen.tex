...
\TODO{start here, fill in tables, explain b for mass and b/beta for fissile}
...
commodity. A mass-based constraint is provided for each requesting
facility. Recycling facilities use the same 800 ton per year limit discussed in
\S \ref{method:setup:front}. Repostories, however, use a limit based on the
Yucca Mountain statutory limit of 17,000 tons and assuming a 30-year operating
lifetime, i.e., period of time in which fuel can enter the facility. Thus, a
monthly limit is determined to be

\begin{equation}
b^{\text{proc}}_r = \sim 48 \frac{\text{t}}{\text{month}}.
\end{equation}

\paragraph{Fissile Content Constraint}

In addition to the mass-based processing constraint, an additional fissile
quantity constraint is added for each recycling facility. The constraining value
for the fissile quantity constraint is determined based on the primary reactor
each supporting facility supports. The value is determined based on the average
fissile enrichment for primary reactors as well as the target elemental
composition of reactor fuel, as shown in Equation \ref {}. Applying this to each
recycling facility type, the values for $b^{\text{fiss}}_r$ are shown in Table
\ref{}. The associated constraint coefficient values, $\beta_r$, are based on
the fissile content of the proposed material transfer, as shown in Equation \ref
{}.

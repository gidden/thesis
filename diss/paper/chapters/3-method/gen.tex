...
parameters generation methodology associated with front-end exchanges. 
\secref{method:setup:back} follows with a similar discussion for back-end
exchanges.  
...
[0, 1)$, that represents a unique measure of quality of fuel requested by
  reactors. This stochastic property is expounded upon further in
  \secref{method:setup:front} and \ref{method:setup:back}.
...
power. Support facilities for front-end exchanges are described in 
\secref{method:setup:front:sup}, and support facilities for back-end exchanges are
...
result in two (or more) manageable problem instances. If a truly inseparable
...
supporting facility population is determined, as described in 
\secref{method:setup:front:sup}. Conceptually, the information gathering procedure
...
phase is modeled using the location proxy described in 
\secref{method:setup:features:prefs}. Throughout the discussion on generating

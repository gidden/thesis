...
commodity, $h$, and supplier, $s$, is denoted $K_s^h$; a requster's
commodity-based constraints is denoted $K_r^h$. Each constraint has a
constraining value, $b_s^{h, k}$ and $b_r^{h, k}$, respectively. Additionally,
each unique combination of portfolio, commodity, and constraint has an
associated \textit{constraint coefficient conversion function}, denoted
$\beta_s^{h, k}$ for supply portfolios and $\beta_r^{h, k}$ for request
portfolios. Each constraint coefficient conversion function takes as an argument
a resource \textit{quality}, $q_{i,j}$. A clarifying example of the relation
between portfolios, commodities, constraints, and coefficient conversion
functions is provided in \S \ref{abm:dre:fctp:arcs}.
...
necessity due to the complexity of the DRE. An example will help clarify its
purpose.
...
process, $b_{s}^{EU,SWU}$, and the total natural uranium (NU) feed it has on hand.,
$b_{s}^{EU,NU}$. The constraint set for $s$ is then
 
\begin{equation}\label{eqs:enr-constr-commods}
  K_{s}^{EU} = \{ \mbox{SWU}, \mbox{NU} \}.
\end{equation}
...
\beta_{s}^{EU, NU}(\varepsilon_{j}) = & \:\: \frac{\varepsilon_{j} - \varepsilon_{t,s}}
                                      {\varepsilon_{f,s} - \varepsilon_{t,s}} \\
\begin{split}
\label{eqs:enr-swu-beta}
\beta_{s}^{EU, SWU}(\varepsilon_{j}) = & \:\: V(\varepsilon_{j}) \\
                         & + \frac{\varepsilon_{j} - \varepsilon_{f,s}}
                                  {\varepsilon_{f,s} - \varepsilon_{t,s}} V(\varepsilon_{t,s}) \\
                         & - \frac{\varepsilon_{j} - \varepsilon_{t,s}}
                                  {\varepsilon_{f,s} - \varepsilon_{t,s}} V(\varepsilon_{f,s})
...
\sum_{j \in J}\beta_{s}^{EU,NU}(\varepsilon_{j}) \: x_{s,j}^{EU}  & \leq b_{s}^{EU,NU} \\
\label{eqs:enr-swu-constr}
\sum_{j \in J}\beta_{s}^{EU,SWU}(\varepsilon_{j}) \: x_{s,j}^{EU} & \leq b_{s}^{EU,SWU}
\end{align}

...
portfolios, and is termed the \textit{Response to Request for Bids} (RRFB)
phase. Each consumption portfolio is comprised of requests for some set of
...
quantity. At the completion of the RRFB completion the possible
connections between supplier and producer facilities, i.e., the arcs in the
graph of the transportation problem, have been established with specific capacity
constraints defined both by the quantity of commodities that will traverse the
arcs but also by the quality.
